...
# ===== TELEGRAM CONFIG =====
BOT_TOKEN = "8638614279:AAGjRYjqA_xiCCdv7Soncf7Ip1yrzkgvT8E"
CHAT_IDS = [
    "672314873",   # Your account
    "357104976"    # Second account
]
...
def send_message(msg):
    ...
    for chat_id in CHAT_IDS:
        try:
            ...
            pass
if "startup_message_sent" not in st.session_state:
    send_message("🚀 Wealth Wave Scanner Started\n\n✅ Telegram Connected\n✅ EMA75 Alert Engine Active")
    st.session_state.startup_message_sent = True

if "ema_alerts" not in st.session_state:
    st.session_state.ema_alerts = set()
...
if st.sidebar.button("📲 Test Telegram"):
    send_message("🧪 Wealth Wave Telegram Test Successful")
...
for _, r in data.iterrows():
    try:
        distance = abs((r["CMP"] - r["EMA75"]) / r["EMA75"]) * 100
        if distance <= 0.5:
            key = r["Symbol"]
            if key not in st.session_state.ema_alerts:
                send_message(
                    f"🎯 EMA75 Pullback Alert\n\nStock: {r['Symbol']}\nCMP: ₹{r['CMP']}\nEMA75: ₹{r['EMA75']}\nDistance: {distance:.2f}%"
                )
                st.session_state.ema_alerts.add(key)
    except:
        pass
...
ema75_df = data[
    (abs((data["CMP"] - data["EMA75"]) / data["EMA75"]) * 100 <= 1)
]
...
with tabs[6]:
    st.success("Stocks trading within 1% of EMA75. Ideal pullback watchlist.")
    st.dataframe(ema75_df.sort_values("CMP"), use_container_width=True)
...
plot_theme = "plotly_dark" if theme == "dark" else "plotly"
